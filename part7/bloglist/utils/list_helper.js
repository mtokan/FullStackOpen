...
const mostBLogs = (blogs) => {
  ...
  const [authorWithMaxBlogs, maxBlogCount] = Array.from(authorCountsMap.entries()).reduce((max, current) =>
...
const mostLikes = (blogs) => {
  ...
  const [authorWithMostLikes, totalLikes] = Array.from(likesCountsMap.entries()).reduce((max, current) =>
    max[1] > current[1] ? max : current,
  )
